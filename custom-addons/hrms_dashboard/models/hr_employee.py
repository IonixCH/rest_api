...
class HrEmployee(models.Model):
    ...
    def get_department_leave(self):
        ...
        for line in results:
            ...
            # Skip if employee doesn't exist or has no calendar
            ...
            try:
                ...
                # If still having calendar issues, use simple day calculation
    ...
    def get_work_days_dashboard(self, from_datetime, to_datetime,
        ...
        # Ensure calendar exists, if not create default one
        if not calendar:
            calendar = self.env['resource.calendar'].search([('name', '=', 'Standard 40 hours/week')], limit=1)
            if not calendar:
                calendar = self.env['resource.calendar'].create({
                    'name': 'Standard 40 hours/week',
                    'hours_per_day': 8.0,
                })
            # Update employee with default calendar if none exists
            if not self.resource_calendar_id:
                self.resource_calendar_id = calendar
        
        # Ensure resource exists
        if not resource:
            # Create default resource for employee
        ...
        # Simple fallback if calendar operations still fail
        try:
            ...
            # Fallback: simple day calculation
            return (to_datetime.date() - from_datetime.date()).days + 1
